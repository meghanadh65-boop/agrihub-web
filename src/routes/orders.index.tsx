...
type Order = { id: string; created_at: string; total: number; status: string; payment_method: string };
type Booking = { id: string; created_at: string; service_slug: string; provider_name: string; acres: number; scheduled_date: string; total: number; status: string };
...
function OrdersPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [o, b] = await Promise.all([
        supabase.from("orders").select("id, created_at, total, status, payment_method").order("created_at", { ascending: false }),
        supabase.from("bookings").select("id, created_at, service_slug, provider_name, acres, scheduled_date, total, status").order("created_at", { ascending: false }),
      ]);
      setOrders(o.data ?? []);
      setBookings(b.data ?? []);
    })();
  ...
  if (loading || !user || orders === null || bookings === null) {
    return (
      <SiteShell>
        <div className="grid min-h-[400px] place-items-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
      </SiteShell>
    );
  }
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        <Tabs defaultValue="orders">
          ...
          <TabsContent value="orders" className="mt-6 space-y-3">
            {orders.length === 0 && <EmptyState label="No orders yet." link="/shop" cta="Start shopping" />}
            {orders.map((o) => (
              <Link key={o.id} to="/orders/$id" params={{ id: o.id }}>
                <Card className="flex flex-col justify-between gap-3 p-5 transition-shadow hover:shadow-soft sm:flex-row sm:items-center">
                  <div>
                    <div className="text-xs uppercase text-muted-foreground">Order · {new Date(o.created_at).toLocaleDateString()}</div>
                    <div className="mt-1 font-mono text-sm">{o.id.slice(0, 8).toUpperCase()}</div>
                    <div className="mt-1 text-xs text-muted-foreground">Payment: {o.payment_method.toUpperCase()}</div>
                  </div>
          ...
          <TabsContent value="bookings" className="mt-6 space-y-3">
            {bookings.length === 0 && <EmptyState label="No bookings yet." link="/services" cta="Book a service" />}
            {bookings.map((b) => (
              <Link key={b.id} to="/orders/booking/$id" params={{ id: b.id }}>
                <Card className="flex flex-col justify-between gap-3 p-5 transition-shadow hover:shadow-soft sm:flex-row sm:items-center">
                  <div>
                    <div className="text-xs uppercase text-muted-foreground">Booking · {new Date(b.created_at).toLocaleDateString()}</div>
                    <div className="mt-1 font-semibold capitalize">{b.service_slug} · {b.provider_name}</div>
                    <div className="mt-1 text-xs text-muted-foreground">{b.acres} acres · {new Date(b.scheduled_date).toLocaleDateString()}</div>
                  </div>
                  <div className="flex items-center gap-4">
...
function EmptyState({ label, link, cta }: { label: string; link: string; cta: string }) {
  return (
    <Card className="p-8 text-center">
      <p className="text-muted-foreground">{label}</p>
      <Link to={link} className="mt-3 inline-block font-semibold text-primary hover:underline">{cta} →</Link>
    </Card>
  );
}
