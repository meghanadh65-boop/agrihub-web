...
export const Route = createFileRoute("/services/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Book agriculture services on demand — drone spraying, harvester rental, tractor booking, and skilled workforce." },
      { property: "og:title", content: "FLTY Agri Services" },
      { property: "og:description", content: "On-demand drone, harvester, tractor and workforce services." },
    ],
  }),
            ...
            <Card key={s.slug} className="overflow-hidden">
              <div className="grid gap-0 md:grid-cols-2">
                <div className="relative h-56 md:h-full">
                  <img src={imgs[s.slug]} alt={s.name} loading="lazy" width={1280} height={800} className="h-full w-full object-cover" />
                </div>
                <div className="p-6">
                  ...
                  <Link to="/services/$service" params={{ service: s.slug }}>
                    <Button className="mt-5 gap-1.5">Learn more <ArrowRight className="h-4 w-4" /></Button>
                  </Link>
                </div>
              </div>
            </Card>
      ...
      <section className="bg-accent/40 py-14">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-4">
            {["Choose a service", "Enter your farm details", "Confirm date & operator", "Get the job done"].map((step, i) => (
