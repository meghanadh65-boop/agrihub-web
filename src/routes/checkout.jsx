...
function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      setForm((f) => ({
        ...f,
        contact_phone: user.phone ? "+" + user.phone.replace(/^\+/, "") : f.contact_phone,
      }));
    }
              ...
              >
                ...
                <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4 has-[:checked]:border-primary has-[:checked]:bg-accent/50">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      Scan a QR at the door — no cash needed.
                    </div>
                  </div>
                </label>
              </RadioGroup>
          ...
          <Card className="h-fit p-6 lg:sticky lg:top-24">
            ...
            <dl className="mt-4 space-y-1 border-t border-border pt-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd>{shipping ? `₹${shipping}` : <span className="text-primary">FREE</span>}</dd>
              </div>
...
function Field({ label, id, value, onChange, required = true }) {
  return (
    <div>
