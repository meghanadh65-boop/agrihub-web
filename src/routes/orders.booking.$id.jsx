...
function BookingDetail() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "status_events", filter: `entity_id=eq.${id}` },
        () => loadAll(),
      )
      .on(
        "postgres_changes",
    ...
    <SiteShell>
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        <div className="mt-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-xs uppercase text-muted-foreground capitalize">
              {booking.service_slug} booking
            </div>
            <h1 className="mt-1 text-2xl font-bold">{booking.provider_name}</h1>
            <div className="mt-1 text-sm text-muted-foreground">
              {booking.acres} acres · <Calendar className="mr-0.5 inline h-3.5 w-3.5" />
            ...
            {STEPS.map((s, idx) => {
              ...
              return (
                <li key={s} className="relative mb-6 last:mb-0">
                  <span
                    className={`absolute -left-[33px] grid h-6 w-6 place-items-center rounded-full border-2 ${reached ? "border-primary bg-primary" : "border-border bg-background"}`}
                  >
                    {reached ? (
          ...
          {nextStep && (
            <div className="mt-4 border-t border-dashed border-border pt-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Demo: in production, providers push these updates.
              </p>
            </div>
          )}
        </Card>
        ...
        <Card className="mt-6 p-6">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 text-sm">
            <div>
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                <Phone className="h-4 w-4" /> {booking.contact_phone}
              </div>
            </div>
            <div>
              <div className="text-xs uppercase text-muted-foreground">Location</div>
              <div className="mt-1 flex items-start gap-2">
                <MapPin className="mt-0.5 h-4 w-4 text-primary" /> {booking.village}
              </div>
            </div>
            <div>
              <div className="text-xs uppercase text-muted-foreground">Rate</div>
              <div className="mt-1">
                ₹{booking.price_per_acre} × {booking.acres}
              </div>
            </div>
            <div>
          ...
          {booking.notes && (
            <div className="mt-4 rounded-md bg-accent/50 p-3 text-xs">{booking.notes}</div>
          )}
        </Card>
      </div>
    </SiteShell>
