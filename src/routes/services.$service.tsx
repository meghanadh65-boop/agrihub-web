...
export const Route = createFileRoute("/services/$service")({
  ...
  errorComponent: () => <SiteShell><div className="p-10 text-center">Error loading service.</div></SiteShell>,
});
  ...
  return (
    <SiteShell>
      <section className="relative overflow-hidden">
        <img src={imgs[service.slug]} alt={service.name} width={1280} height={800} className="absolute inset-0 h-full w-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-foreground/85 to-foreground/40" />
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6">
          <span className="inline-flex rounded-full bg-brand-yellow px-3 py-1 text-xs font-semibold text-foreground">FLTY Service</span>
          <h1 className="mt-4 text-4xl font-extrabold text-background sm:text-5xl">{service.name}</h1>
                ...
                <Card key={p.name} className="flex items-center justify-between p-5">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> {p.area}</span>
