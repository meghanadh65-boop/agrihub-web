...
export function BookingForm({ service, providers }) {
  const { t } = useI18n();
  ...
  return (
    <Card className="p-6 md:p-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Pick a provider and confirm — team calls within 2 hours.
      </p>
        ...
        <div>
          ...
          <div className="mt-2 space-y-2">
            {providers.map((p) => (
              <label
                key={p.id}
                className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 transition-colors ${providerId === p.id ? "border-primary bg-accent/50" : "border-border hover:border-primary/50"}`}
              >
                <input
                  type="radio"
                  name="provider"
          ...
          <div className="rounded-lg bg-accent/70 p-4">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">
                ₹{provider.price_per_acre} × {acres} {provider.price_unit}s
              </span>
